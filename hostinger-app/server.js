...
async function paidBuyer(req,res,next){try{req.buyer=jwt.verify(req.cookies.pezo_buyer,JWT_SECRET);const u=await db.one('SELECT paid FROM users WHERE email=$1',[req.buyer.email]);if(!u?.paid)return res.status(403).json({detail:'Tài khoản chưa được mở quyền.'});next()}catch{return res.status(401).json({detail:'Bạn cần đăng nhập lại.'})}}
...
app.get('/workspace',buyer,wrap(async(req,res)=>{const u=await db.one('SELECT paid FROM users WHERE email=$1',[req.buyer.email]);if(!u?.paid)return res.redirect('/checkout');const url=await db.getSetting('dashboard.url','/workspace');if(url&&url!=='/workspace')return res.redirect(url);res.sendFile(path.join(__dirname,'public','tool','index.html'))}));
app.get('/tool/result.html',paidBuyer,(req,res)=>res.sendFile(path.join(__dirname,'public','tool','result.html')));
app.get('/api/buyer/me',async(req,res)=>{try{const u=jwt.verify(req.cookies.pezo_buyer,JWT_SECRET),row=await db.one('SELECT paid FROM users WHERE email=$1',[u.email]);res.json({loggedIn:true,paid:!!row?.paid})}catch{res.json({loggedIn:false,paid:false})}});
